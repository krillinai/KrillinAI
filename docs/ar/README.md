<div align="center">
  ...
  # أداة ترجمة الفيديو والصوت الذكية
...
Krillin AI هو حل شامل لتوطين وتعزيز الصوت والفيديو. هذه الأداة البسيطة والقوية تجمع بين ترجمة الفيديو، والتعليق الصوتي، واستنساخ الصوت، وتدعم إخراج بتنسيقات عمودية وأفقية، مما يضمن عرضًا مثاليًا على جميع المنصات الرئيسية (Bilibili، Xiaohongshu، Douyin، WeChat Video، Kuaishou، YouTube، TikTok، إلخ). من خلال سير العمل من البداية إلى النهاية، يمكن لـ Krillin AI تحويل المواد الأصلية إلى محتوى متعدد المنصات جاهز للاستخدام بنقرات قليلة فقط.

## الميزات والوظائف الرئيسية:
🎯 **تشغيل بنقرة واحدة**: لا حاجة لتكوين بيئة معقدة، يتم تثبيت الاعتماديات تلقائيًا، ويمكنك البدء على الفور، مع إصدار سطح مكتب جديد لمزيد من الراحة!
...
🌍 **ترجمة احترافية**: تعتمد على LLM، مع الحفاظ على تماسك المعنى على مستوى الفقرات

🎙️ **استنساخ الصوت**: يوفر أصوات مختارة من CosyVoice أو استنساخ صوت مخصص

🎬 **تركيب الفيديو**: معالجة تلقائية للفيديوهات الأفقية والعمودية وتنسيق الترجمة
...
## 🚀 البدء السريع
### الخطوات الأساسية
أولاً، قم بتنزيل [الإصدار](https://github.com/krillinai/KrillinAI/releases) الذي يتوافق مع نظام جهازك، ثم اختر ما إذا كنت تريد إصدار سطح المكتب أو غير سطح المكتب، ثم ضع الملفات في مجلد فارغ، حيث سيتم إنشاء بعض الدلائل بعد التشغيل، لذا من الأفضل إدارتها في مجلد فارغ.  

【إذا كان إصدار سطح المكتب، أي أن ملف الإصدار يحمل كلمة desktop، انظر هنا】  
_إصدار سطح المكتب هو إصدار جديد، تم إصداره لحل مشكلة عدم قدرة المستخدمين الجدد على تحرير ملفات التكوين بشكل صحيح، ولا يزال هناك العديد من الأخطاء، جاري التحديث_
1. انقر نقرًا مزدوجًا على الملف لبدء الاستخدام (يحتاج إصدار سطح المكتب أيضًا إلى تكوين داخل البرنامج)

【إذا كان إصدار غير سطح المكتب، أي أن ملف الإصدار لا يحمل كلمة desktop، انظر هنا】  
_إصدار غير سطح المكتب هو الإصدار الأول، تكوينه معقد نسبيًا، ولكنه مستقر، ومناسب للنشر على الخوادم، حيث سيتم توفير واجهة المستخدم عبر الويب_
1. أنشئ مجلدًا باسم `config` داخل المجلد، ثم أنشئ ملفًا باسم `config.toml` داخل مجلد `config`، انسخ محتوى ملف `config-example.toml` الموجود في دليل `config` وألصقه في `config.toml`، واملأ معلومات التكوين الخاصة بك.
...
### إلى: مستخدمي macOS
【إذا كان إصدار سطح المكتب، أي أن ملف الإصدار يحمل كلمة desktop، انظر هنا】  
حاليًا، بسبب مشاكل في التوقيع، لا يمكن تشغيل إصدار سطح المكتب بنقرة مزدوجة أو تثبيته عبر dmg، تحتاج إلى الوثوق بالتطبيق يدويًا، الطريقة كالتالي:
...
* اختر `openai` لكل من `transcription_provider` و`llm_provider`، بحيث تحتاج فقط إلى ملء `openai.apikey` في الفئات الثلاث أدناه `openai` و`local_model` و`aliyun` للقيام بترجمة الترجمة. (يمكن ملء `app.proxy` و`model` و`openai.base_url` حسب الحاجة)

استخدام نموذج التعرف على الصوت المحلي (غير مدعوم حاليًا على macOS) (توازن بين التكلفة والسرعة والجودة)
* املأ `transcription_provider` بـ `fasterwhisper` و`llm_provider` بـ `openai`، بحيث تحتاج فقط إلى ملء `openai.apikey` و`local_model.faster_whisper` في الفئات الثلاث أدناه `openai` و`local_model` للقيام بترجمة الترجمة، وسيتم تنزيل النموذج المحلي تلقائيًا. (مثل `app.proxy` و`openai.base_url` كما هو مذكور أعلاه)

في الحالات التالية، تحتاج إلى تكوين Alibaba Cloud:
* إذا تم ملء `llm_provider` بـ `aliyun`، فستحتاج إلى استخدام خدمة النموذج الكبير من Alibaba Cloud، لذا تحتاج إلى تكوين `aliyun.bailian`.
* إذا تم ملء `transcription_provider` بـ `aliyun`، أو إذا تم تفعيل وظيفة "التعليق الصوتي" عند بدء المهمة، فستحتاج إلى استخدام خدمة الصوت من Alibaba Cloud، لذا تحتاج إلى ملء `aliyun.speech`.
* إذا تم تفعيل وظيفة "التعليق الصوتي" وتم تحميل صوت محلي لاستنساخ الصوت، فستحتاج أيضًا إلى استخدام خدمة تخزين السحابة OSS من Alibaba Cloud، لذا تحتاج إلى ملء `aliyun.oss`.  
...
## معايير المساهمة
1. لا تقدم ملفات غير مفيدة مثل .vscode و.idea، يرجى استخدام .gitignore للتصفية
2. لا تقدم config.toml، بل استخدم config-example.toml لتقديمه

## اتصل بنا
1. انضم إلى مجموعة QQ الخاصة بنا، للإجابة على الأسئلة: 754069680
2. تابع حساباتنا على وسائل التواصل الاجتماعي، [Bilibili](https://space.bilibili.com/242124650)، حيث نشارك محتوى عالي الجودة في مجال التكنولوجيا AI يوميًا
