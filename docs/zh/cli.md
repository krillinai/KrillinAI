...
| `voices` | 音色查询 | 查看 TTS provider 可用的 `--voice` 代码 |
...
列出 TTS provider 可用的音色代码，用于 `tts --voice`。

```bash
krillinai voices [flags]
```

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|
| `--provider` | string | 否 | 当前配置 | TTS provider，可选 `aliyun`、`openai`、`edge-tts` |
| `--dry-run` | bool | 否 | `false` | 返回本地音色清单，不执行外部调用 |

**示例：**

```bash
# 查看阿里云 TTS 常用音色代码
krillinai voices --provider aliyun
```

输出中的 `voices[].code` 可直接传给 `tts --voice`。

---

### 4. render-horizontal / render-vertical — 视频合成
