...
func configurePackagedCoreDependencies() error {
	...
	storage.YtdlpPrefixArgs = nil
	if command := os.Getenv("OPENCREATOR_YT_DLP_COMMAND"); command != "" {
		var args []string
		if err := json.Unmarshal([]byte(command), &args); err != nil || len(args) == 0 || !filepath.IsAbs(args[0]) {
			return fmt.Errorf("invalid OPENCREATOR_YT_DLP_COMMAND")
		}
		info, err := os.Stat(args[0])
		if err != nil || !info.Mode().IsRegular() {
			return fmt.Errorf("yt-dlp interpreter is unavailable: %s", args[0])
		}
		storage.YtdlpPath = args[0]
		storage.YtdlpPrefixArgs = append([]string{}, args[1:]...)
		return nil
	}
